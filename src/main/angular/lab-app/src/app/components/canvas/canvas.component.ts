...
export class CanvasComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  drawShapes(ctx: CanvasRenderingContext2D, center: number, rOffset: number) {
    ...
    // прямоугольник
    ctx.fillRect(center - rOffset, center - rOffset * 0.5, rOffset, rOffset * 0.5);
    ...
    ctx.beginPath();
    ctx.lineTo(center + rOffset, center);
    ctx.arc(center, center, rOffset/2, 0, Math.PI / 2);
    ctx.lineTo(center, center);
    ctx.fill();
  }
  ...
  drawHits(ctx: CanvasRenderingContext2D) {
    ...
    hits.forEach(point => {
      let isHit: boolean;
      isHit = point.result == "Попала";
      this.drawHit(ctx, point.x, point.y, design.colors[isHit ? 'hit' : 'miss'])
    });
  }
